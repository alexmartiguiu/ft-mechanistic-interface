...
const Mono =({ children, block }) => (
  <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: block ? "13px" : "12.5px", color: "var(--ink)", background: "var(--card-2)", border: "1px solid var(--line)", borderRadius: "9px", padding: block ? "12px 14px" : "8px 11px", lineHeight: 1.6, overflowX: "auto", whiteSpace: "pre" }}>{children}</div>
);
...
export default function SteeringMathArtifact({ concept, mitigate }) {
  ...
  const glyph = <ArtifactGlyph tone="var(--seal)" bg="var(--seal-soft)"><Sigma size={16} strokeWidth={1.8} /></ArtifactGlyph>;
  const update = `h_ℓ  ←  h_ℓ ${coef < 0 ? "−" : "+"} ${Math.abs(coef)} · v̂_${c}`;

  const expanded = (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px", fontSize: "13.5px", color: "var(--ink-soft)", lineHeight: 1.65 }}>
      <div>
        <div style={{ fontWeight: 600, color: "var(--ink)", marginBottom: "6px" }}>1 · Concept direction</div>
        The <span style={{ fontFamily: "'JetBrains Mono',monospace", color: "var(--seal)" }}>{c}</span> direction is the difference of mean residual-stream activations between contrastive prompt sets at layer ℓ, unit-normalized:
        <div style={{ marginTop: "8px" }}><Mono block>{`v_ℓ = mean[ h_ℓ(x⁺) ]  −  mean[ h_ℓ(x⁻) ]\nv̂_ℓ = v_ℓ / ‖v_ℓ‖`}</Mono></div>
      </div>
      {combined && (
        <div>
          <div style={{ fontWeight: 600, color: "var(--ink)", marginBottom: "6px" }}>2 · Combined with the universal basis</div>
          This run uses <span style={{ fontFamily: "'JetBrains Mono',monospace", color: "var(--seal)" }}>method: combined</span> — the domain direction is reconciled with the shared universal safety vectors {"{ û_k }"} so steering rides the basis the vectors were jointly validated on:
          <div style={{ marginTop: "8px" }}><Mono block>{`v̂* = normalize(  v̂_ℓ  +  Σ_k ⟨v̂_ℓ, û_k⟩ · û_k  )`}</Mono></div>
        </div>
      )}
      <div>
        <div style={{ fontWeight: 600, color: "var(--ink)", marginBottom: "6px" }}>{combined ? "3" : "2"} · Preventive steering (training-time hook)</div>
      ...
      <div>
        <div style={{ fontWeight: 600, color: "var(--ink)", marginBottom: "6px" }}>{combined ? "4" : "3"} · Drift monitor</div>
    ...
    <Artifact glyph={glyph} title="Preventive steering" subtitle={`${c} · coef ${coef} · ${sign}`} expanded={expanded} expandedSubtitle={`how v̂_${c} is computed & applied`}>
      <div style={{ display: "flex", flexDirection: "column", gap: "9px" }}>
        <div style={{ fontSize: "12.5px", color: "var(--mute)" }}>
          Steered the <b style={{ color: "var(--ink)" }}>{c}</b> direction during training{combined ? " (combined with the universal basis)" : ""} to prevent drift:
        </div>
        <Mono>{update}</Mono>
      </div>
    </Artifact>
